...
# Base environment
        ...
        cd shekyl/utils/build_scripts
        docker build -f android32.Dockerfile -t shekyl-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t shekyl-gui-android .
        docker create -it --name shekyl-gui-android shekyl-gui-android bash

# Get the apk

        docker cp shekyl-gui-android:/opt/android/shekyl-core/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/shekyl-core/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
